...
describe('fetch:copier', () => {
  ...
  beforeEach(() => {
    ...
    mockContext = {
      ...
      templateInfo: {
        entityRef: 'template:default/copier',
        baseUrl: 'somebase',
      },
  ...
  it('should call fetchContents with the correct variables', async () => {
    ...
    expect(fetchContents).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        outputPath: join(mockTmpDir, 'template', "{{copier and 'contents'}}"),
      }),
    );
  });

  it('should call out to copier using executeShellCommand when copier is installed', async () => {
  ...
  it('should call out to the containerRunner when there is no copier installed', async () => {
    ...
    expect(containerRunner.runContainer).toHaveBeenCalledWith(
      expect.objectContaining({
        imageName: 'spotify/backstage-copier',
        command: 'copier',
